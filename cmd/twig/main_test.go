...
func TestCleanCmd_TargetCompletion(t *testing.T) {
	t.Parallel()
	...
	testutil.RunGit(t, mainDir, "branch", "feat/b")

	cmd := newRootCmd()
	cmd.SetArgs([]string{"-C", mainDir, "__complete", "clean", "--target", ""})
	...
	// Check that branches are included in completion
	expectedBranches := []string{"main", "develop", "feat/a", "feat/b"}
	for _, branch := range expectedBranches {
		if !strings.Contains(output, branch) {
			t.Errorf("completion output %q should contain branch %q", output, branch)
		}
	}

	// Check that ShellCompDirectiveNoFileComp is set (value 4)
	if !strings.Contains(output, ":4") {
		t.Errorf("completion output %q should contain ':4' (ShellCompDirectiveNoFileComp)", output)
	}
}
